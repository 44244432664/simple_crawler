...
class WikiCrawler:
        ...
        if os.path.exists(f"{self.output_dir}/{self.novel_info['title']}_info.json"):
            ...
            if existing_info["title"] == self.novel_info["title"] and \
               existing_info["author"] == self.novel_info["author"] and \
                ...
                if len(existing_info["chapter_links"]) == self.novel_info["num_chapters"]:
                    print("Novel information is the same as the existing one, using existing data.")
                    self.novel_info = existing_info
                else:
                    print("Novel information is the same, but some chapter links are missing, updating chapter links...")
                    self.novel_info["chapters"] = existing_info["chapters"]
    ...
    def __book_info_json__(self):
        ...
        return f"{self.output_dir}/{self.novel_info['title']}_info.json"


def control_Wikidich_crawler(crawler, action, show_info_after=False, make_book_option=2, *args):
    # print(f"Args: {args}")
    crawler.get_novel_info_()
    # crawler.get_all_chapters_links_()
    if action == "get_all":
        ...
        for arg in args[0]:
            # print(f"Getting chapter: {arg}")
            crawler.get_chapter(arg)
        ...
        crawler.get_chapter_range(*args)
    elif action == "exit":
    ...
    if make_book_option == 1:
        json_path = crawler.__book_info_json__()
        if action == "get_all":
            for i in tqdm.tqdm(range(1, crawler.novel_info["num_chapters"] + 1), desc="Creating EPUBs", unit="book"):
                # crawler.make_chapter_book(i)
                create_epub(json_path, crawler.output_dir, chapter_num=[i])
        elif action == "get_chapter":
            # crawler.make_chapter_book(chapter)
            for a in args[0]:
                # print(f"Creating EPUB for chapter: {a}")
                create_epub(json_path, crawler.output_dir, chapter_num=[a])
        elif action == "get_chapter_range":
            start, end = args
            for i in tqdm.tqdm(range(int(start), int(end) + 1), desc="Creating EPUBs", unit="book"):
                # crawler.make_chapter_book(i)
                create_epub(json_path, crawler.output_dir, chapter_num=[i])
    elif make_book_option == 2:
        # crawler.make_all_book()
        if action == "get_all":
            create_epub(json_path, crawler.output_dir)
        elif action == "get_chapter":
            create_epub(json_path, crawler.output_dir, chapter_num=args[0])
        elif action == "get_chapter_range":
            create_epub(json_path, crawler.output_dir, chapter_num=(args[0], args[1]))
